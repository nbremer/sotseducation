...
var	chosen = "HBO",
	...
	provincies = ["Drenthe", "Flevoland", "Friesland", "Gelderland", "Groningen", "Limburg", "N. Brabant", "N. Holland", "Overijssel", "Utrecht", "Zeeland", "Z. Holland"];
...
//Draw the windroses				
drawRoses(roseData, roseDiagonal, provincies, roseRadius, roseColors);
...
function drawRoses(data, diagonal, provincies, radius, color) {
	...
	function updateRoses() {
		...
		for(x in data[chosen]) {
			var npath = d3.select("#pie"+x).selectAll("path")
						.data(pie(data[chosen][x]));
...
function showRoseTooltip (d, i, j) {
  $(this).popover({
	...
	html : true,
	content: function() { 
	  return "<span style='font-size: 11px;'><span style='color: #00A1DE; font-weight: bold;'>" + numFormatPercentDec(roseData[chosen][j][i]) + "</span>" + 
