...
/*Slight adjustment for the Chord Diagram*/
function wrapChord(text, width) {
  text.each(function() {
	  
	if (d3.select(this).text() === "Administratief personeel") var widthFinal = 70;
	else var widthFinal = width;
    ...
    while (word = words.pop()) {
      line.push(word);
      tspan.text(line.join(" "));
      if (tspan.node().getComputedTextLength() > widthFinal) {
        line.pop();
        tspan.text(line.join(" "));
        line = [word];
        tspan = text.append("tspan").attr("x", x).attr("y", y).attr("dy", ++lineNumber * lineHeight + dy + "em").text(word);
